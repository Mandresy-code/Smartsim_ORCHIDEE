...
def write(varlist, resultpath, IDx):
    ...
    if mode == "unstructured":
       for year in range(varlist["climate"]["year_start"], varlist["climate"]["year_end"] + 1):
           print("Building unstructured forcing for year %s" % year)
        ...
        # Open source dataset
           ...
           #ncout = Dataset(resultpath + "forcing_unstructured_" + str(year) + ".nc", "w")
        ...
        # Define dimensions
           ...
           ncout.createDimension("nvertex", 4)  # Set to 4 for potential polygon support
        ...
        # Copy attributes from source
           ...
           ncout.variables["nav_lon"][:] = [xlon for _, xlon in IDx]
           ncout.variables["nav_lat"][:] = [xlat for xlat, _ in IDx]
        ...
        # Process forcing variables
           for var in nc.variables:
               if var in ["nav_lon", "nav_lat", "time", "timeplussix"]:
               ...
               ncout.createVariable(var, nc.variables[var].dtype, newdims, fill_value=9.96921e36)
               ...
               if len(nc.variables[var].dimensions) == 3:
                   ...
                   selected_data = np.array([ncdata[:, np.where(lat == xlat)[0][0], np.where(lon == xlon)[0][0]] for xlat, xlon in IDx])
